...
## 2026-06-14 — MinioStorage 반영 워커 이미지 amd64 재빌드 + GHCR 푸시
- 결정: MinioStorage가 반영된 현재 코드로 워커 이미지를 다시 빌드해 `ghcr.io/lucid9512/doongle-ai:latest` 를 갱신한다(기존 이미지는 MinioStorage 이전 구버전).
- 결과: Mac(arm64)에서 `docker buildx build --platform linux/amd64 ... --load` 로 amd64 이미지 빌드 성공. 빌드 로그상 `minio-7.2.20` 이 순수 파이썬 휠(py3-none-any)로 설치됨(컴파일 0). torch 는 베이스 이미지에 이미 있어 재설치되지 않음. GHCR 푸시 완료(digest sha256:c89fe537d722...).
- 비고: `requirements.txt` 의 `minio>=7.2.0` 한 줄로 충분(추가 빌드 의존성 없음). Dockerfile/의존성은 변경하지 않았다.

## 2026-06-15 — api/워커 DB 드라이버 상이(async/sync) → 워커 전용 DATABASE_URL_SYNC로 분리
- 배경: api(async, asyncpg)와 워커(sync, psycopg2)가 같은 PostgreSQL을 쓰지만 드라이버가 달라 URL 스킴이 다르다(`postgresql+asyncpg://` vs `postgresql://`). k8s에서 `DATABASE_URL`(=async 스킴) 하나를 공유하니, 동기 워커가 그 URL로 엔진을 만들며 asyncpg를 import하려다 ModuleNotFoundError로 죽었다.
- 결정: 워커는 DB URL을 **전용 환경변수 `DATABASE_URL_SYNC`**(sync 스킴)에서 읽는다. api는 그대로 `DATABASE_URL` 사용 → 키가 분리돼 충돌 없음. 워커만 변경.
- 구현: config.py의 DB URL 읽기를 `_get("DATABASE_URL_SYNC", required=True)`로 변경(읽는 지점은 한 곳뿐). dataclass 필드명은 `database_url` 유지(워커 코드가 `settings.database_url`로 참조 — 환경변수 키만 변경). redacted()/주석/.env.example도 DATABASE_URL_SYNC로 맞춤.
- 검증: 옛 `DATABASE_URL`만 있으면 기동 실패(required), `DATABASE_URL_SYNC`로는 정상 로드(비밀번호 마스킹) 확인. 이미지 amd64 재빌드(--no-cache) + GHCR 푸시.